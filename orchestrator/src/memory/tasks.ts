...
export type Task = { id: string; finding_id: string; priority: 'P0'|'P1'|'P2'|'P3'; title: string; description: string; acceptance_criteria: string[]; assigned_to: 'codex'; status: 'queued'|'working'|'testing'|'verified'|'failed' };
...
export function taskFromFinding(finding: Finding, index: number): Task {
  return { id: `TASK-${String(index).padStart(3, '0')}`, finding_id: finding.id, priority: finding.severity, title: finding.title, description: `${finding.problem}\n\nRecommendation: ${finding.recommendation}`, acceptance_criteria: [finding.expected, 'The affected page loads without browser console errors.'], assigned_to: 'codex', status: 'queued' };
}
