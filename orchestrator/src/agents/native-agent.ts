...
async function context() {
  return Promise.all(['PROJECT.md', 'PRODUCT_SPEC.md', 'ARCHITECTURE.md', 'CURRENT_STATE.md'].map(file => readFile(path.resolve('.agent', file), 'utf8')));
}
...
export async function implement(task: Task): Promise<ImplementationResult> {
  ...
  if (task.finding_id === 'F-002') {
    const file = path.resolve('public/js/app.js');
    ...
    await writeFile(file, source.replace(unsafe, safe));
...
export async function runChecks(): Promise<{ build: boolean; tests: boolean; output: string[] }> {
  const output: string[] = [];
  try {
    await exec(process.execPath, ['--check', 'server.js']);
    output.push('Node syntax validation passed for server.js.');
  } catch (error: any) {
    output.push(error.stderr || error.message);
    return { build: false, tests: false, output };
  }

  try {
    await exec(process.execPath, ['--check', 'public/js/app.js']);
    output.push('Node syntax validation passed for public/js/app.js.');
    return { build: true, tests: true, output };
  } catch (error: any) {
    output.push(error.stderr || error.message);
    return { build: true, tests: false, output };
  }
}
